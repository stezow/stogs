...
namespace  {
    // all scintillators
    ScintillatorDF theScintillatorFactory("Scintillators/");
}

void ScintillatorDF::MakeStore()
{        
// PARIS RELATED
...
G4VPhysicalVolume * ScintillatorDF::Make(G4String name, G4String version_string)
{
...
// PARIS RELATED
    if ( name == "ParisPW" ) {
        ...
        if ( version_string.contains("2") ) {
            if ( version_string == "2" ) {
                theDetector = MakePPW(detname,0.5*mm,0.0*mm,"");
            }
            if ( version_string.contains("2-bare") ) {
                theDetector = MakePPW(detname,0.5*mm,0.0*mm,"bare");
            }
        }
        if ( version_string.contains("3") ) {
            if ( version_string == "3" ) {
                theDetector = MakePPW(detname,1.0*mm,0.0*mm,"");
            }
            if ( version_string.contains("3-bare") ) {
                theDetector = MakePPW(detname,1.0*mm,0.0*mm,"bare");
            }
        }
    }
    if ( name == "CParisPW" ) {
...
G4VPhysicalVolume *ScintillatorDF::MakeCdC(G4String detname, G4String opt)
{
    G4VPhysicalVolume *theDetector = 0x0; G4LogicalVolume *detlogicWorld; G4Box *detWorld; G4bool do_caps = true;

    // from manifacture
	const G4double caps_width = 0.9*mm, teflon_width = 0.1*mm, crystal_radius = 5.0*cm, crystal_depth = 14*cm;
	...
	detWorld= new G4Box(detname,15.*cm,15.*cm,25.*cm);
	...
	// crystal
	G4Polyhedra* a_solid
        = new G4Polyhedra("C2CCrys",0.,360.1*deg,numSide,numZPlanes,z,rInner,rOuter);
	//Setting a logical volume
	G4LogicalVolume *a_log
        = new G4LogicalVolume(a_solid,ParisMaterialConsultant::theConsultant()->GetMaterial("BaF2"),"C2CCrys",0,0,0);
	
	G4VisAttributes *visatt = new G4VisAttributes( G4Colour(0.0, 0.6, 0.0) );
	visatt->SetVisibility(true);
	a_log->SetVisAttributes( visatt );
    a_log->SetSensitiveDetector( ParisOutputManager::GetTheOutputManager()->GetCaloSD() );
    ...
    if ( do_caps ) {
        ...
        // encapsulation side
        a_solid = new G4Polyhedra("C2CCapsSide",0.,360.*deg,numSide,numZPlanes,z_caps,rInner_caps,rOuter_caps);
        //Setting a logical volume
        a_log
            = new G4LogicalVolume(a_solid,ParisMaterialConsultant::theConsultant()->GetMaterial("Aluminium"),"C2CCapsSide",0,0,0);
        ...
        a_solid = new G4Polyhedra("C2CCapsFront",0.,360.*deg,numSide,numZPlanes,z_caps_front,rInner_caps_front,rOuter_caps_front);
        //Setting a logical volume
        a_log
            = new G4LogicalVolume(a_solid,ParisMaterialConsultant::theConsultant()->GetMaterial("Aluminium"),"C2CCapsFront",0,0,0);
...
G4VPhysicalVolume *ScintillatorDF::MakePPW(G4String detname, G4double caps_width, G4double housing_width, G4String opt)
{
	...
	G4double
        InnerX = 2*2.54*cm, InnerY = 2*2.54*cm, InnerZ = 2*2.54*cm,
        OuterX = 2*2.54*cm, OuterY = 2*2.54*cm, OuterZ = 6*2.54*cm;    
    // in case of housing longer at the back of the PW
	G4double ExtraHousingBack = 20*mm, eps_caps =  caps_width / 100. , eps_housing = 0.05*mm; // eps remove in the capsule part to avoid overlapping
   // G4double r_caps_width = caps_width - eps_caps, r_housing_width = housing_width - eps_housing;
	...
	G4LogicalVolume *Inner_logic =
        new G4LogicalVolume(Inner_solid, ParisMaterialConsultant::theConsultant()->GetMaterial(matInner),"PW:0",0,0,0);
    ...
    Inner_logic->SetSensitiveDetector( ParisOutputManager::GetTheOutputManager()->GetCaloSD() );
	...
	G4LogicalVolume *Outer_logic =
        new G4LogicalVolume(Outer_solid,ParisMaterialConsultant::theConsultant()->GetMaterial(matOuter), "PW:1",0,0,0);
    ...
    Outer_logic->SetSensitiveDetector( ParisOutputManager::GetTheOutputManager()->GetCaloSD() );
	...
	if ( do_caps ) {
        ...
        zplane[0] = 0.01*mm; zplane[1] = caps_width - eps_caps; zplane[2] = InnerZ + OuterZ + ExtraHousingBack;
        rinner[0] = 0.00*mm; rinner[1] = InnerX / 2 + eps_caps; rinner[2] = InnerX / 2 + eps_caps;
        ...
        //router[0] = router[1] = router[2] = InnerX / 2 + caps_width;
 		...
 		G4Polyhedra *side_part =
            new G4Polyhedra("PwCaps", 0.0, 361.0*deg, 4, 3, zplane, rinner, router);
        T.setX( 0.0 );
        T.setY( 0.0 );
        T.setZ( 0.0 );
        ...
        Ro->rotateZ(45.0*deg);
 		...
 		G4LogicalVolume *Capsule_logic_side =
            new G4LogicalVolume(side_part,ParisMaterialConsultant::theConsultant()->GetMaterial(matCapsule),"PWCaps",0,0,0);
		...
		G4LogicalVolume *Capsule_logic =
            new G4LogicalVolume(Capsule_solid,ParisMaterialConsultant::theConsultant()->GetMaterial(matCapsule),"PWCapsule",0,0,0);
	...
	if ( do_housing ) {
		...
		G4LogicalVolume *Housing_logic =
            new G4LogicalVolume(Housing_solid,ParisMaterialConsultant::theConsultant()->GetMaterial(matHousing),"PWHousing",0,0,0);
...
G4VPhysicalVolume *ScintillatorDF::MakeCPPW(G4String detname, G4String opt)
{
    G4VPhysicalVolume *theDetector = 0x0; G4LogicalVolume *detlogicWorld; G4Box *detWorld; G4double caps_width = 0.5*mm;
    G4RotationMatrix R;
	G4ThreeVector T;
    G4Transform3D Tr;
    ...
    // use a physical as a container to describe the detector
	detWorld= new G4Box(detname,20.*cm,20.*cm,25.*cm);
	detlogicWorld= new G4LogicalVolume(detWorld, ParisMaterialConsultant::theConsultant()->GetMaterial("Air"), detname, 0, 0, 0);
    ...
    if ( opt.contains("3") ) {
        caps_width = 1.0*mm; // version 3 have PW with caps of 1 mm
    }
    
    const G4double HalfInnerX = 2.54*cm + 1.001*caps_width, HalfInnerY = 2.54*cm + 1.001*caps_width; // 
    ...
    // remap assembly
    DoMap(assembly,a_pw,ParisOutputManager::GetGCopyNb());
